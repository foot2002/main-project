...
import { PORTFOLIO_CATEGORIES } from "@/types/admin-portfolio";
import type { PortfolioItem, PortfolioFormPayload, PortfolioStatus } from "@/types/admin-portfolio";
...
interface PortfolioFormProps {
  ...
  onSubmit: (payload: PortfolioFormPayload) => void;
  submitting?: boolean;
}

const emptyPayload: PortfolioFormPayload = {
  title: "",
  content: "",
  category: "",
  imageFile: null,
...
export function PortfolioForm({ open, onOpenChange, initialData, onSubmit, submitting = false }: PortfolioFormProps) {
  ...
  useEffect(() => {
    if (open) {
      if (initialData) {
        ...
        setWrittenDate(initialData.writtenDate ? initialData.writtenDate.slice(0, 10) : "");
        ...
        setWrittenDate(emptyPayload.writtenDate);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit Portfolio" : "Create New Portfolio"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label htmlFor="portfolio-title">Title (required)</Label>
            <input
              id="portfolio-title"
              type="text"
              value={title}
          ...
          <div>
            <Label htmlFor="portfolio-content">Content (required)</Label>
            <textarea
          ...
          <div>
            <Label htmlFor="portfolio-category">Category (required)</Label>
            <select
          ...
          <div>
            <Label htmlFor="portfolio-image">Image {isEdit ? "(optional, leave empty to keep current)" : "(required)"}</Label>
            <input
              id="portfolio-image"
              type="file"
              accept="image/*"
          ...
          <div>
            <Label htmlFor="portfolio-author">Author (optional)</Label>
            <input
              id="portfolio-author"
              type="text"
              value={author}
          ...
          <div>
            <Label htmlFor="portfolio-written-date">Written Date (optional)</Label>
            <input
              id="portfolio-written-date"
              type="date"
              value={writtenDate}
